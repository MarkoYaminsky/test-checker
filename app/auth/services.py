...
def create_jwt_token(user_id: UUID, token_type: JWTTokenType) -> str:
    token_type_to_expiration_delta = {
        ...
        JWTTokenType.websocket: timedelta(minutes=1),
    }
...
def get_ws_authenticated_user(db: Session, token: str) -> Optional[User]:
    payload = decode_jwt_token(token)
    return validate_jwt_token_payload(db=db, payload=payload, token_type=JWTTokenType.websocket)
